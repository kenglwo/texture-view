...
router.get("/", (req: Request, res: Response, next: NextFunction) => {
  ...
  const thanksWord = query.thanksWord;
  const offset = 0;
    ...
    try {
      const result = await client.query(
        ...
        select
          ...
          m.description,
          m.advice,
          count(t.thanks_word) as thanksWord_count,
        ...
        group by
          ...
          m.description,
          m.advice
        having count(t.thanks_row) > 0
        order by count(t.thanks_row) desc
        limit 10 offset $3 ;`,
        [`%${keyword}%`, thanksWord, offset]
      );
      ...
      _.forEach(result.rows, elem => {
        ...
        const RECIPE_JSON = {
          ...
          thanksWord_count: elem.thanksWord_count
        };
